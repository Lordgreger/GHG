...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DropableNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DropableNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DropableNetworkObject.Length > 0 && DropableNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DropableNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DropableBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDropable instead, its shorter and easier to type out ;)")]
		public DropableBehavior InstantiateDropableNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DropableNetworkObject[index]);
			var netBehavior = go.GetComponent<DropableBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DropableBehavior>().networkObject = (DropableNetworkObject)obj;
		...
		public DropableBehavior InstantiateDropable(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DropableNetworkObject[index]);
			var netBehavior = go.GetComponent<DropableBehavior>();
